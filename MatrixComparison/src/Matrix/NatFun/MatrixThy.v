...
(*     Bind Scope mat_scope with mat. *)
  ...
  Section meq.
    ...
    Proof. intros r c [g]. intros i j Hi Hj. auto. Qed.
    ...
    Lemma meq_dec_equiv : forall {r c} (m1 m2 : M r c), 
      {meq m1 m2} + {~(meq m1 m2)}.
    Proof. intros r c [g1] [g2]. simpl. apply seq2eq_dec. Qed.
    ...
    Section AXIOM_FOR_REWRITE_TEMPORARY_SOLUTION.
      ...
      Lemma meq_imply_eq : forall {r c} (m1 m2 : M r c),
        meq m1 m2 -> m1 = m2.
      Proof. intros. apply meq_iff. auto. Qed.
      
      Lemma eq_imply_meq : forall {r c} (m1 m2 : M r c),
        m1 = m2 -> meq m1 m2.
      Proof. intros; subst. reflexivity. Qed.
    
      Lemma mneq_imply_neq : forall {r c} (m1 m2 : M r c),
        ~(meq m1 m2) -> m1 <> m2.
      Proof. intros. intro. apply meq_iff in H0. auto. Qed.
      
      Lemma neq_imply_mneq : forall {r c} (m1 m2 : M r c),
        m1 <> m2 -> ~(meq m1 m2).
      Proof. intros; subst. intro. apply meq_imply_eq in H0. auto. Qed.
...
(*   Notation "m1 == m2" := (meq m1 m2) (at level 70) : mat_scope. *)
  ...
  Section mrow.
    ...
    *)
    Fixpoint mrow_aux {r c : nat} (ri : nat) (cnt : nat) (m : M r c) 
      : list X := match c with
      | O => nil
      | S c' =>
        let g := mdata m in
    ...
    Definition mrow {r c : nat} (ri : nat) (m : M r c) := 
      @mrow_aux r c ri 0 m.
  ...
  Section cvt_lst_and_mat.
    ...
    Lemma m2l_aux_length : forall {r c} cnt (m : M r c), 
      length (m2l_aux cnt m) = r.
    Proof.
      induction r; auto. intros. simpl. destruct r; auto.
    Qed.
    
    Lemma m2l_aux_width : forall {r c} cnt {m : M r c}, 
  ...
  Section mcol.
    ...
    (* Aux function, cnt initial is 0 *)
    Fixpoint MCol_aux (r c : nat) (ci : nat) (cnt : nat) (m : M r c) 
      : list X := match r with
      | O => nil
      | S r' =>
        let g := (mdata m) in
  ...
  Section Mat1_Mat0.
    ...
    (* Zero M *)
    Definition mat0 (r c : nat) : M r c :=
      mk_mat _ _ (fun _ _ => X0).
  ...
  Section mcmul_props.
    ...
    Lemma mcmul_1_r : forall {n} a, a c* mat1 n =
  ...
  Section mmul_props.
    ...
    Lemma mmul_1_l : forall {r c : nat} (m : M r c), 
      mat1 r * m = m.
    Proof.
    ...
    Lemma mmul_1_r : forall {r c : nat} (m : M r c), 
      m * mat1 c = m.
    Proof.
      intros m n [g]; apply meq_iff; intros i j Hi Hj. simpl.
    ...
    Lemma mcmul_mul_distr_l : forall {r c s} (a : X) 
      (m1 : M r c) (m2 : M c s), 
      (a c* m1) * m2 = a c* (m1 * m2).
    Proof.
    ...
    Lemma mcmul_mul_distr_r : forall {r c s} (a : X) 
      (m1 : M r c) (m2 : M c s), 
      m1 * (a c* m2) = a c* (m1 * m2).
    Proof.
      intros r c s a [g1] [g2]; apply meq_iff; intros i j _ _ . simpl.
