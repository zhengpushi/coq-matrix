...
Fixpoint perm_aux {X} (X0 : X) (n : nat) (l : list X) : list (list X) :=
  ...
  | 0 => [[]]
  | S n' =>
    let d1 := map (fun i => pick X0 l i) (seq 0 n) in
    let d2 := map 
      (fun k : X * list X => let (x, lx) := k in
...
(*   Fixpoint det {n} : Square n -> X :=
  ...
  Proof. intros. cbv. f_equal;f_equal. field. auto. Qed.
  ...
  Proof. intros. cbv. tac_listeq; try field; auto. Qed.
...
(*     Compute dl_T2R (dl_nsimp l3 50). (* *)
    ...
    *)
    ...
    (* 先给出3x3矩阵行列式的表达式 *)
    Let det_m3_exp := r11*r22*r33 - r11*r23*r32 - r12*r21*r33 + r12*r23*r31 + r13*r21*r32 - r13*r22*r31.
    ...
    Proof. clear l1 l2 l3. intros. field.
...
(*     Check nth 0 (mat_vars_s 3 4).
    ...
    (** 矩阵的生成函数 *)
    Definition g' r c : MATData T := fun i j =>
      (TLit (env_s2R (nth j (nth i (mat_vars_s r c) []) ""))).
...
(*     Print mat_vars_given. *)
    ...
    Definition g : MATData T := fun i j =>
      (TLit (env_s2R (nth j (nth i mat_vars_given []) ""))).
