...
Require Export DepList.MatrixThy.
...
Module VectorThy (F : FieldSig) <: VectorThySig F.
  ...
  Definition t2v_2 (t : @T2 X) := let '(a,b) := t in [[a];[b]].
  Definition t2v_3 (t : @T3 X) := let '(a,b,c) := t in [[a];[b];[c]].
  Definition t2v_4 (t : @T4 X) := let '(a,b,c,d) := t in [[a];[b];[c];[d]].
  ...
  Definition v2l' {n} (v : V n) : list X := to_list (mcoli v F1).
